...
The core system prompt located inside `src/app/api/chat/route.ts` contains highly specific psychological directives designed to push back on startup ideas ruthlessly.
**DO NOT** overwrite, simplify, or abstract the system prompt away from its profound 5-Level GRIT cognitive framework. It must maintain the "Hardened CTO" identity. It contains strict `BANNED BEHAVIOR` constraints against summarizing user input, using AI fluff, or becoming a "Template Zombie" (forcing rigid markdown structures). Any code refactoring around the Next.js routing **must preserve** these hardcoded prompt mechanics precisely as written.
